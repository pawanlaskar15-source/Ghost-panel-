...
# ================= CONFIG =================
BOT_TOKEN = "8715354967:AAElEp3MVTHNJiaIrIK0QAJBhzyDivnpgMo"
BOT_OWNER = 7944283616

# Required Channels for join verification
REQUIRED_CHANNELS = [
    {"username": "ddosserverx", "url": "https://t.me/ddosserverx", "id": -1001234567890},  # Replace with actual channel ID
    {"username": "godxddos", "url": "https://t.me/godxddos", "id": -1001234567891}  # Replace with actual channel ID
]
...
DEFAULT_API_KEY = "8D99BDA9CCD74511EC7571FE20561C94"

DEFAULT_MAX_ATTACK_TIME = 400
DEFAULT_COOLDOWN = 80
DEFAULT_MAX_CONCURRENT = 13  # CHANGED: Max slots 13
PORT_BLOCK_DURATION = 7200  # 2 hours
AUTO_RESET_INTERVAL = 60000  # 1000 minutes for auto-reset
MAX_RETRIES = 3
RETRY_DELAY = 5
# ==========================================
...
channel_verify_col = get_collection("channel_verify")  # NEW: For tracking channel verification
...
def get_channel_id(username):
    ...
    try:
        ...
        logger.error(f"Could not get channel ID for @{username}: {e}")
        return None

def update_channel_ids():
    """Update channel IDs in the config"""
    global REQUIRED_CHANNELS
    for channel in REQUIRED_CHANNELS:
        try:
            chat = bot.get_chat(f"@{channel['username']}")
            channel['id'] = chat.id
            logger.info(f"Updated channel ID for @{channel['username']}: {chat.id}")
        except Exception as e:
            logger.error(f"Could not update channel ID for @{channel['username']}: {e}")
...
def check_all_channels_joined(user_id):
    ...
    for channel in REQUIRED_CHANNELS:
        ...
        if channel_id:
            if not is_user_joined_channel(user_id, channel_id):
                not_joined.append(channel)
        else:
            # Try to get channel ID dynamically
            try:
                chat = bot.get_chat(f"@{channel['username']}")
                channel['id'] = chat.id
                if not is_user_joined_channel(user_id, chat.id):
                    not_joined.append(channel)
            except:
                not_joined.append(channel)
...
def get_channel_verify_keyboard():
    ...
    keyboard.add(
        InlineKeyboardButton("🔄 Check Again", callback_data="check_channels")
    )
    return keyboard

def get_channel_verify_text():
    """Get channel verification text"""
    text = (
        "🔐 <b>CHANNEL VERIFICATION REQUIRED</b>\n"
        "╔══════════════════════════════╗\n"
        "║   🔐 ACCESS REQUIREMENT     ║\n"
        "╚══════════════════════════════╝\n\n"
        "📢 <b>To use this bot, you must join both of our channels:</b>\n\n"
    )
    
    for channel in REQUIRED_CHANNELS:
        text += f"• <a href='{channel['url']}'>{channel['username']}</a>\n"
    
    text += (
        "\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "📋 <b>How to verify:</b>\n"
        "1️⃣ Click the buttons below to join both channels\n"
        "2️⃣ Click <b>'✅ I've Joined Both Channels'</b>\n"
        "3️⃣ You'll get full access to the bot!\n\n"
        "⚠️ <b>Note:</b> You must join both channels to use the bot.\n"
...
def check_channel_verification(user_id):
    ...
    try:
        record = channel_verify_col.find_one({"_id": str(user_id)})
        if record and record.get("verified", False):
            # Check if verification is still valid (optional: add expiry)
            return True
        return False
    except:
        return False
...
def is_authorized(user_id, chat_id=None):
    # Check if owner or admin first (bypass all checks)
...
def check_access(message):
    ...
    if not check_channel_verification(user_id):
        not_joined = check_all_channels_joined(user_id)
        if not_joined:
            # User hasn't joined channels, show verification message
            keyboard = get_channel_verify_keyboard()
            bot.reply_to(message, get_channel_verify_text(), reply_markup=keyboard, disable_web_page_preview=True)
            return False
        else:
            # User has joined channels, mark as verified
    ...
    try:
        ...
        if call.data == "verify_channels":
            ...
            if not not_joined:
                ...
                keyboard = get_channel_verify_keyboard()
                text = get_channel_verify_text()
                text += f"\n\n⚠️ <b>Still Not Joined:</b>\n"
            ...
            if not not_joined:
                ...
                bot.answer_callback_query(call.id, "❌ Please join both channels!", show_alert=True)
                bot.edit_message_text(
                    text,
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
...
def handle_attack(message):
    try:
        ...
        # CHANGED: Use DEFAULT_MAX_CONCURRENT = 13
