    ...
    try:
        if settings_col.count_documents({}) == 0:
            settings_col.insert_one({
                ...
                "channel_verification_required": True  # NEW: Channel verification toggle
            })
...
def is_user_joined_channel(user_id, channel_id):
    ...
    try:
        ...
        # User is considered joined if status is not 'left' or 'kicked'
        return status not in ['left', 'kicked']
    except telebot.apihelper.ApiException as e:
        logger.warning(f"Error checking channel membership for {channel_id}: {e}")
        # If we can't check, assume they are joined (to avoid blocking)
        return True
    except Exception as e:
...
def check_access(message):
    ...
    if not is_group(message):
        ...
        # Channel verification for private chat
...
def start_command(message):
    try:
        ...
        # Check channel verification for non-owner users
        if is_channel_verification_enabled() and not is_owner(user_id) and not is_admin(user_id):
            if not verify_user_channels(user_id):
                missing = check_all_channels_joined(user_id)
                if missing:
...
def required_command(message):
    try:
        ...
        if status == "on":
            ...
            # Clear all user verification records
