...
BOT_OWNER_2 = 7896370578  # Added secondary owner
...
DEFAULT_MAX_ATTACK_TIME = 400  # CHANGED: 400 seconds
DEFAULT_COOLDOWN = 80
DEFAULT_MAX_CONCURRENT = 13  # CHANGED: 13 concurrent
    ...
    try:
        if settings_col.count_documents({}) == 0:
            settings_col.insert_one({
                ...
                "max_concurrent_attacks": 13,  # CHANGED: 13 concurrent
                "port_protection": False,
                "feedback_system": True,  # CHANGED: Enabled
...
def execute_attack(target, port, duration):
    ...
    for method_name, field in combos:
        payload = {"ip": target, "port": port, "duration": duration, field: method_name}
        logger.info(f"[DEBUG] Trying method={method_name}, field={field}")
        try:
            resp = session.post(api_url, json=payload, headers=headers, timeout=timeout)
            if resp.status_code in [200, 201, 202, 204]:
                logger.info(f"[DEBUG] ✅ Success with {method_name}/{field}")
                return resp
            else:
                err = resp.text[:100] if resp.text else ""
                logger.info(f"[DEBUG] Failed {resp.status_code}: {err}")
        except Exception as e:
            logger.info(f"[DEBUG] Exception: {e}")
            continue
    
    # Fallback: try GET method with query params
    try:
        full_url = f"{api_url}?key={api_key}&target={target}&port={port}&time={duration}&method={method}"
        resp = session.get(full_url, timeout=15)
        if resp.status_code in [200, 201, 202, 204]:
            logger.info("[DEBUG] ✅ Success with GET method")
            return resp
    except:
        pass
...
def start_attack(target, port, duration, message, attack_id, cooldown_seconds):
    try:
        ...
        # Always consider attack successful if we got a response
        attack_sent = True
    ...
    try:
        ...
        try:
            ...
            if BOT_OWNER_2 != BOT_OWNER:
                bot.send_message(BOT_OWNER_2, forward_msg, parse_mode="HTML")
        ...
        log_error(e, user_id=message.from_user.id, command="report")
        bot.reply_to(message, "❌ An error occurred. Please try again.")
...
def keyreset_command(message):
    try:
        ...
        if not original_days:
            original_hours = key.get("duration_hours")
            if original_hours:
        ...
        logger.error(f"Error in keyreset command: {e}")
        log_error(e, user_id=message.from_user.id, command="keyreset")
        bot.reply_to(message, "❌ An error occurred. Please try again.")
...
def keyblock_command(message):
    try:
        ...
        if original_days <= 0:
            original_hours = key.get("duration_hours", 0)
            if original_hours > 0:
                original_days = original_hours / 24
            else:
        ...
        logger.error(f"Error in keyblock command: {e}")
        log_error(e, user_id=message.from_user.id, command="keyblock")
        bot.reply_to(message, "❌ An error occurred. Please try again.")
...
def remove_reseller(message):
    try:
        ...
        # Delete all keys created by this reseller
        keys_col.delete_many({"created_by": user_id})
        ...
        bot.reply_to(message, 
                    ...
                    f"🗑 All keys created by this reseller have been deleted.")
...
def broadcast_command(message):
    try:
        ...
        for uid in users:
            try:
                if photo:
                    bot.send_photo(uid, photo, caption=broadcast_msg)
                else:
...
def gentrial_command(message):
    try:
        ...
        bot.reply_to(message, 
                    ...
                    f"📅 <b>Expires:</b> {format_expiry(expires)}\n"
                    f"🎯 <b>Attacks per key:</b> Unlimited\n"
...
# ================= DELETE ALL TRIALS COMMAND =================
@bot.message_handler(commands=['deletealltrials'])
def delete_all_trials_command(message):
    try:
        if not is_admin_or_owner(message.from_user.id):
            bot.reply_to(message, "❌ Only admins or owner can use this command.")
            return
        ...
        log_admin_action(message.from_user.id, "deletealltrials", f"Deleted {result.deleted_count} all trial keys")
        
        bot.reply_to(message, 
                    f"✅ <b>All Trial Keys Deleted</b>\n\n"
                    f"🗑 Deleted: {result.deleted_count} trial keys (including used ones)\n\n"
                    f"All trial keys have been removed.")
        
    except Exception as e:
        logger.error(f"Error in deletealltrials command: {e}")
        log_error(e, user_id=message.from_user.id, command="deletealltrials")
        bot.reply_to(message, "❌ An error occurred. Please try again.")
...
def block_code_command(message):
    try:
        ...
        if key.get("redeemed_by"):
            # Revoke access if already redeemed
            redeemed_by = key.get("redeemed_by")
            plans_col.delete_one({"_id": str(redeemed_by)})
            clear_pending_feedback(redeemed_by)
            try:
                bot.send_message(redeemed_by, f"🚫 Your access key {code} has been blocked by the admin.")
            except:
                pass
        ...
        bot.reply_to(message, 
                    ...
                    f"If it was redeemed, the user's access has been revoked.")
...
def key_state_command(message):
    try:
        ...
        text = (
            ...
            f"🔄 <b>Reset Count:</b> {key.get('reset_count', 0)}/2"
        )
...
def owner_panel(message):
    try:
        ...
        panel_text = (
            ...
            f"/reseller_info\n\n"
            f"🔹 <b>Key Management:</b>\n"
            f"/genkey /gentrial /gentrialfor /deletetrials /deletealltrials\n"
            f"/list_codes /delete_code /block_code /key_state\n\n"
            f"🔹 <b>Settings:</b>\n"
            ...
            try:
                ...
                if code:
                    ...
                    if key and key.get("duration_days") and key.get("created_at"):
                        ...
                        new_expiry = min(expires + timedelta(seconds=seconds), max_expiry)
                        plans_col.update_one(
                            {"_id": user["_id"]},
                            {"$set": {"expires": new_expiry.isoformat()}}
                        )
                        count += 1
                        continue
...
def deduct_time_command(message):
    try:
        if not is_admin_or_owner(message.from_user.id):
            bot.reply_to(message, "❌ Only admins or owner can use this command.")
            return
        ...
        log_admin_action(message.from_user.id, "deduct_time", f"Deducted {seconds}s from user {redeemed_by}")
        
        bot.reply_to(message, f"✅ Deducted {seconds} seconds from user {redeemed_by}'s plan.")
        
    except Exception as e:
        logger.error(f"Error in deduct_time command: {e}")
        log_error(e, user_id=message.from_user.id, command="deduct_time")
        bot.reply_to(message, "❌ An error occurred. Please try again.")
...
def private_users_command(message):
    try:
        ...
        if len(plans) > 50:
            text += f"\n⚠️ Showing first 50 users. Total: {len(plans)}"
        
        bot.reply_to(message, text[:4000])
        
    except Exception as e:
        logger.error(f"Error in private_users command: {e}")
        log_error(e, user_id=message.from_user.id, command="private_users")
        bot.reply_to(message, "❌ An error occurred. Please try again.")
    ...
    try:
        ...
        if len(parts) != 3:
            bot.reply_to(message, "⚠️ Usage: /setapi &lt;api_url&gt; &lt;api_key&gt;\n\n"
                        "Example: /setapi https://web-production-c4df.up.railway.app/api/v1/attack your_api_key")
            return
        ...
        log_admin_action(message.from_user.id, "setapi", f"API URL: {api_url}, Key: {api_key[:10]}...")
...
def set_time_command(message):
    try:
        ...
        plans_col.update_many({}, {"$set": {"max_duration": value}})
        ...
        bot.reply_to(message, 
                    ...
                    f"📋 All existing user plans have been updated.")
...
def set_cooldown_command(message):
    try:
        ...
        # Update all existing plans
        plans_col.update_many({}, {"$set": {"cooldown": value}})
...
def maintenance_command(message):
    try:
        ...
        if status == "on":
            ...
            # Calculate downtime and extend users
            ...
            if start_time_str:
                ...
                if seconds > 0:
                    ...
                    try:
                        ...
                        for user in plans_col.find({"expires": {"$gt": datetime.now().isoformat()}}):
                            try:
...
def view_logs_command(message):
    try:
        ...
        logger.error(f"Error in view_logs command: {e}")
        log_error(e, user_id=message.from_user.id, command="view_logs")
        bot.reply_to(message, "❌ An error occurred. Please try again.")

# ================= VIEW CODE LOGS COMMAND =================
@bot.message_handler(commands=['view_code_logs'])
def view_code_logs_command(message):
    try:
        if not is_admin_or_owner(message.from_user.id):
            bot.reply_to(message, "❌ Only admins or owner can view this.")
            return
        ...
        logger.error(f"Error in view_code_logs command: {e}")
        log_error(e, user_id=message.from_user.id, command="view_code_logs")
        bot.reply_to(message, "❌ An error occurred. Please try again.")

# ================= DELETE LOGS COMMANDS =================
@bot.message_handler(commands=['deletelogs_attack'])
def delete_attack_logs_command(message):
    try:
        if not is_admin_or_owner(message.from_user.id):
            bot.reply_to(message, "❌ Only admins or owner can use this command.")
            return
        
        result = attack_logs_col.delete_many({})
        log_admin_action(message.from_user.id, "deletelogs_attack", f"Deleted {result.deleted_count} attack logs")
        
        bot.reply_to(message, f"✅ Deleted {result.deleted_count} attack logs.")
        
    except Exception as e:
        logger.error(f"Error in deletelogs_attack command: {e}")
        log_error(e, user_id=message.from_user.id, command="deletelogs_attack")
        bot.reply_to(message, "❌ An error occurred. Please try again.")

@bot.message_handler(commands=['deletelogs_key'])
def delete_key_logs_command(message):
    try:
        if not is_admin_or_owner(message.from_user.id):
            bot.reply_to(message, "❌ Only admins or owner can use this command.")
            return
        
        result = key_logs_col.delete_many({})
        log_admin_action(message.from_user.id, "deletelogs_key", f"Deleted {result.deleted_count} key logs")
        
        bot.reply_to(message, f"✅ Deleted {result.deleted_count} key logs.")
        
    except Exception as e:
        logger.error(f"Error in deletelogs_key command: {e}")
        log_error(e, user_id=message.from_user.id, command="deletelogs_key")
        bot.reply_to(message, "❌ An error occurred. Please try again.")

@bot.message_handler(commands=['deletelogs_all'])
def delete_all_logs_command(message):
    try:
        if not is_admin_or_owner(message.from_user.id):
            bot.reply_to(message, "❌ Only admins or owner can use this command.")
            return
        
        attack_count = attack_logs_col.delete_many({}).deleted_count
        key_count = key_logs_col.delete_many({}).deleted_count
        admin_count = admin_logs_col.delete_many({}).deleted_count
        
        log_admin_action(message.from_user.id, "deletelogs_all", f"Deleted {attack_count} attack, {key_count} key, {admin_count} admin logs")
        
        bot.reply_to(message, f"✅ Deleted:\n• Attack logs: {attack_count}\n• Key logs: {key_count}\n• Admin logs: {admin_count}")
        
    except Exception as e:
        logger.error(f"Error in deletelogs_all command: {e}")
        log_error(e, user_id=message.from_user.id, command="deletelogs_all")
        bot.reply_to(message, "❌ An error occurred. Please try again.")
